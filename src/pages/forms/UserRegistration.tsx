...
const UserRegistration = () => {
  ...
  useEffect(() => {
    ...
    const loadDocNo = async () => {
      try {
        const newDocNo = await generateDocNo('FM-IT-006', 'userRegistrations');
        if (!cancelled) {
          setWrNumber(newDocNo);
        }
      } catch (error) {
        console.error('Failed to generate FM-IT-006 number:', error);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const latestWrNumber = await generateDocNo('FM-IT-006', 'userRegistrations');
      ...
      await addDoc(collection(db, ROOT_COLLECTION, ROOT_DOCUMENT, 'userRegistrations'), {
        ...data,
    ...
    return (
      <div className="max-w-[95%] mx-auto p-8 md:p-12">
        <div className="glass-card p-10 rounded-2xl shadow-xl text-center">
          ...
          <p className="text-on-surface-variant mb-6">Your user registration request has been saved.</p>
          <button
        ...
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex gap-4">
            <div className="glass-card p-4 rounded-xl min-w-[140px] shadow-sm">
              ...
              <input name="wrNumber" className="w-full bg-transparent border-none p-0 text-xl font-black text-on-surface focus:ring-0 placeholder:opacity-30" placeholder="FM-IT-006-XXXXXXX" type="text" value={wrNumber} readOnly />
            </div>
            <div className="glass-card p-4 rounded-xl min-w-[140px] shadow-sm">
